...
\begin{frame}
\frametitle{Aggregate programming \small [Beal, J. Pianini, D. Viroli, M. (2015)]}
...
\begin{frame}
\centering
\frametitle{Aggregate programming layers \small [Beal, J. Pianini, D. Viroli, M. (2015)]}
\includegraphics[height=7cm]{imgs/layers.pdf}
\end{frame}

\begin{frame}
\frametitle{Field Calculus \small [Viroli, M. Audrito, G. Damiani, F. et al. (2016)]}
\begin{block}{}
...
&& {\color{red} \fifK} (\e) \{\e\} {\color{red} \elseK} \{\e\} \;\BNFmid \; \\
                && {\color{red} \nbrK}\{\e\} \;\BNFmid\; {\color{red} \repK}(\e)\{ (\xname) \ftoSym \e \} \; \BNFmid \; \\
                && {\color{red} \shareK}(\e)\{(\xname) \ftoSym \e \}
                \\[3pt]
...
\begin{block}{Protelis \small [Pianini, D. Montagna, S. Viroli, M. (2013)]}
\begin{multicols}{2}
{Features:
\begin{itemize}
...
def distanceTo(source) {
  rep (POSITIVE_INFINITY) { (d) =>
     mux (source) {
        0
     } else { 
        minHood(nbr(d) + nbrRange.apply())
    }
  }
}
\end{lstlisting}
...
\begin{frame}[fragile]
\frametitle{Existing aggregate frameworks}
\begin{block}{\Scafi [Casadei, R. (2020)]}
\begin{multicols}{2}
{Features:
\begin{itemize}
...
{Limitations:
\begin{itemize}
\item No field calculus semantics
...
def distanceTo(source: Boolean): Double {
  rep (Double.PositiveInfinity) { distance =>
     mux (source) {
        ...
        foldhood(Double.PositiveInfinity)(Math.min(_,_)){ 
...
\frametitle{\FKotac}
\begin{block}{\FKotac}
Subset of the \Kotac{} DSL used in order to formalize syntax, semantics and typing.
